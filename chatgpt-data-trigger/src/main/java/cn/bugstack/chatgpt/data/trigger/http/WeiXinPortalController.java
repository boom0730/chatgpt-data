...
public class WeiXinPortalController {
     ...
     * ！！！！！！！！！！！
     * ！！！！！！！！！！、
     * 这个签名的验证一定要跟 前端需要的校验码以及发送给前端的用户token区别开来
     *
     * 这个验签 其实是微信服务器 和 我这儿的后端服务器之间的验证签名！！！！！！！！！！！
     ...
     * echostr   微信端发来的验证字符串
     */
    @GetMapping(produces = "text/plain;charset=utf-8")//指定的是处理方法返回的内容类型，而不是请求时的参数类型。text/plain 代表返回的是纯文本格式。
     ...
     * ！！！！！！！！！！！！
     * post请求
     * 微信服务器  和  本地的这个服务程序 验证签名 相当于建立了一个可靠双方信任的链接之后
     *
     * 下面这个post 就是 用户通过微信 发送过来的文字  以前做的是 用户可以直接微信使用chatgpt
     * 这里 做的是 用户发送403 之后 就回给他一个校验码 然后去网页前端上进行登录
     */
    @PostMapping(produces = "application/xml; charset=UTF-8")//返回的时xml格式
